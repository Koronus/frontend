...
export function DetailPanel({ onClose, activeMetric }: DetailPanelProps) {
  ...
  // JSON экспорт
  const handleJSONReport = () => {
    const jsonData = {
      title: metricData.title,
      currentValue: metricData.currentValue,
      targetRange: metricData.targetRange,
      status: metricData.status,
      chartData: metricData.chartData,
      problemLocations: metricData.problemLocations,
      relatedIncident: metricData.relatedIncident,
    ...
    const a = document.createElement("a")
    a.href = url
    a.download = `${metricData.title}_${new Date().toISOString().split("T")[0]}.json`
    a.click()
    URL.revokeObjectURL(url)
  }
